...
const SortOrderArrows: React.FC<props> = ({
  sortOrder,
  selected
}) => {
  type arrowsColor = { asc: string; desc: string };
  // const arrowsColor = useRef<arrowsColor>({ asc: '#969696', desc: '#969696' });
  const [arrowsColor,setArrowsColor] = useState<arrowsColor>({ asc: '#969696', desc: '#FFFFFF' });

  useEffect(() => {
    if (selected) {
      if (sortOrder === 'asc') {
        setArrowsColor({ asc: '#ffffff', desc: '#969696' });
      } else if (sortOrder === 'desc') {
        setArrowsColor({ asc: '#969696', desc: '#ffffff' });
      }
    } else {
      setArrowsColor( { asc: '#969696', desc: '#969696' });
    }
  }, [selected, sortOrder]);
